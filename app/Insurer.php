...
class Insurer extends BaseModel
{
    ...
    {
        return $this->hasMany('App\ProviderMalpractice', 'insurer_id');
    }
    ...
    static public function insurersCB($default = [])
    {
        $insurers = self::orderBy('name')->lists('name', 'id')->toArray();

        if (!empty($default)) {
            return self::mergeAssoc($default, $insurers);
        }
