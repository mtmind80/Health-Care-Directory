    ...
    <header id="topbar">
        <div class="topbar-left">
            <ol class="breadcrumb">
                ...
                <li class="crumb-active">
                    <a href="javascript:void(0)" class="no-link">Disciplines</a>
                </li>
    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-discipline'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-discipline'))
                    {!! Form::open(['route' => 'discipline_search_path', 'id' => 'searchForm']) !!}
                    ...
                    @if (!empty($needle))
                        <button class="btn btn-default reset-button equis" type="button" data-location="disciplines">x</button>
                    @endif
                    {!! Form::close() !!}
                @endif
            </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('name', 'Discipline') !!}</th>
                        <th class="actions">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                        @foreach ($disciplines as $discipline)
                            <tr data-id="{{ $discipline->id }}" class="{{ !empty($discipline->disabled) ? 'disabled' : '' }}">
                                <td>
                                    @if (Auth::user()->isAllowTo('update-discipline'))
                                        <span data-toggle="tooltip" title="Click to edit">
                                            <a class="x-editable" href="#"
                                               data-pk="{{ $discipline->id }}"
                                               data-name="name"
                                               data-value="{{ $discipline->name }}"
                                               ...
                                               data-url="{{ route('discipline_inline_update_path') }}">
                                                {{ $discipline->name }}
                                            </a>
                                        </span>
                                    @else
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update-discipline'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="status" data-id="{{ $discipline->id }}">
                                                            @if (empty($discipline->disabled))
                                                ...
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-discipline'))
                                                    <li class="menu-separator"></li>
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="delete" data-id="{{ $discipline->id }}">
        ...
        <div class="row pagination-container clearfix text-center">
            <div class="col-xs-12 col-sm-4">
                <div class="pull-left pagination-info">
                    @if (!$disciplines->total())
                        <span>There is no item to show.</span>
                    @else
                        @if ($disciplines->perPage() > $disciplines->total())
                            <span>Showing {!! $disciplines->total() !!} {{ $disciplines->total() == 1 ? 'item' : 'items' }}</span>
                        @else
                            <span>Showing {{ $disciplines->perPage() }} of {!! $disciplines->total() !!} {{ $disciplines->total() == 1 ? 'item': 'items' }}</span>
                        @endif
                    @endif
                </div>
            </div>
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('discipline_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('discipline_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('discipline_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $disciplines->total()])) }}"{{ Request::input('perPage') == $disciplines->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $disciplines, 'route' => 'discipline_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'discipline_delete_path', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('discipline_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('disciplines') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
