    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-config'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                {!! Form::open(['route' => 'config_search_path', 'id' => 'searchForm']) !!}
                ...
                @if (!empty($needle))
                    <button class="btn btn-default reset-button equis" type="button" data-location="config">x</button>
                @endif
                {!! Form::close() !!}
            </div>
        </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('key', 'Item Identifier') !!}</th>
                        <th class="td-sortable">{!! SortableTrait::link('value', 'Item Value') !!}</th>
                    ...
                    <tbody>
                        @foreach ($conf as $item)
                            <tr data-id="{{ $item->id }}" class="{{ !empty($item->disabled) ? 'disabled' : '' }}">
                                @if (Auth::user()->isAllowTo('modify-config'))
                                    <td><span data-toggle="tooltip" title="Click to edit">
                                        <a class="x-editable" href="#"
                                           ...
                                           data-php-validation-rule="required|identifier|unique:config"
                                           data-type="text"
                                           data-title="Item Identifier:"
                                           data-url="{{ route('config_inline_update_path') }}">
                                            {{ $item->key }}
                                        </a></span>
                                    </td>
                                @else
                                    <td>{{ $item->key }}</td>
                                @endif
                                @if (Auth::user()->isAllowTo('update-config'))
                                    <td><span data-toggle="tooltip" title="Click to edit">
                                        <a class="x-editable" href="#"
                                           data-pk="{{ $item->id }}"
                                           data-name="value"
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update-config'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="status" data-id="{{ $item->id }}">
                                                ...
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-config'))
        ...
        <div class="row pagination-container clearfix text-center">
            <div class="col-xs-12 col-sm-4">
                <div class="pull-left pagination-info">
                    ...
                    @else
                        @if ($conf->perPage() > $conf->total())
                            <span>Showing {!! $conf->total() !!} {{ $conf->total() == 1 ? 'item' : 'items' }}</span>
                        @else
                            <span>Showing {{ $conf->perPage() }} de {!! $conf->total() !!} {{ $conf->total() == 1 ? 'item': 'items' }}</span>
                        @endif
                    @endif
                </div>
            </div>
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('config_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('config_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('config_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $conf->total()])) }}"{{ Request::input('perPage') == $conf->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $conf, 'route' => 'config_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'url' => 'config', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('config_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('config') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
