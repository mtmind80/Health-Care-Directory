    ...
    <header id="topbar">
        <div class="topbar-left">
            <ol class="breadcrumb">
                ...
                <li class="crumb-active">
                    <a href="javascript:void(0)" class="no-link">Document Action Types</a>
                </li>
    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-document-action-type'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-document-action-type'))
                    {!! Form::open(['route' => 'document_action_type_search_path', 'id' => 'searchForm']) !!}
                    ...
                    @if (!empty($needle))
                        <button class="btn btn-default reset-button equis" type="button" data-location="documentactionttypes">x</button>
                    @endif
                    {!! Form::close() !!}
                @endif
            </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('name', 'Document Action Type') !!}</th>
                        <th class="actions">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                        @foreach ($documentActionTypes as $documentActionType)
                            <tr data-id="{{ $documentActionType->id }}" class="{{ !empty($documentActionType->disabled) ? 'disabled' : '' }}">
                                <td>
                                    @if (Auth::user()->isAllowTo('update-document-action-type'))
                                        <span data-toggle="tooltip" title="Click to edit">
                                            <a class="x-editable" href="#"
                                               data-pk="{{ $documentActionType->id }}"
                                               data-name="name"
                                               data-value="{{ $documentActionType->name }}"
                                               ...
                                               data-type="text"
                                               data-title="Document Action Type:"
                                               data-url="{{ route('document_action_type_inline_update_path') }}">
                                                {{ $documentActionType->name }}
                                            </a>
                                        </span>
                                    @else
                                        {{ $documentActionType->name }}
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update-document-action-type'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="status" data-id="{{ $documentActionType->id }}">
                                                            @if (empty($documentActionType->disabled))
                                                ...
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-document-action-type'))
                                                    <li class="menu-separator"></li>
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="delete" data-id="{{ $documentActionType->id }}">
        ...
        <div class="row pagination-container clearfix text-center">
            <div class="col-xs-12 col-sm-4">
                <div class="pull-left pagination-info">
                    @if (!$documentActionTypes->total())
                        <span>There is no item to show.</span>
                    @else
                        @if ($documentActionTypes->perPage() > $documentActionTypes->total())
                            <span>Showing {!! $documentActionTypes->total() !!} {{ $documentActionTypes->total() == 1 ? 'item' : 'items' }}</span>
                        @else
                            <span>Showing {{ $documentActionTypes->perPage() }} of {!! $documentActionTypes->total() !!} {{ $documentActionTypes->total() == 1 ? 'item': 'items' }}</span>
                        @endif
                    @endif
                </div>
            </div>
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('document_action_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('document_action_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('document_action_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $documentActionTypes->total()])) }}"{{ Request::input('perPage') == $documentActionTypes->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $documentActionTypes, 'route' => 'document_action_type_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'document_action_type_delete_path', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('document_action_type_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('documentactionttypes') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
