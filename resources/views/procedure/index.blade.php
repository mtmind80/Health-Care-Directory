    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-procedure'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-procedure'))
                    {!! Form::open(['route' => 'procedure_search_path', 'id' => 'searchForm']) !!}
                    ...
                    @if (!empty($needle))
                        <button class="btn btn-default reset-button equis" type="button" data-location="procedures">x</button>
                    @endif
                    {!! Form::close() !!}
                @endif
            </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('name', 'Procedures') !!}</th>
                    ...
                    <tbody>
                        @foreach ($procedures as $procedure)
                            <tr data-id="{{ $procedure->id }}" class="{{ !empty($procedure->disabled) ? 'disabled' : '' }}">
                                <td>
                                    @if (Auth::user()->isAllowTo('update-procedure'))
                                        <span data-toggle="tooltip" title="Click to edit">
                                            <a class="x-editable" href="#"
                                               data-pk="{{ $procedure->id }}"
                                               data-name="name"
                                               data-value="{{ $procedure->name }}"
                                               ...
                                               data-url="{{ route('procedure_inline_update_path') }}">
                                                {{ $procedure->name }}
                                            </a>
                                        </span>
                                    @else
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update-procedure'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="status" data-id="{{ $procedure->id }}">
                                                            @if (empty($procedure->disabled))
                                                ...
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-procedure'))
        ...
        <div class="row pagination-container clearfix text-center">
            ...
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('procedure_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('procedure_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('procedure_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $procedures->total()])) }}"{{ Request::input('perPage') == $procedures->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $procedures, 'route' => 'procedure_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'procedure_delete_path', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('procedure_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('procedures') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
