    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-action'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-action'))
                    {!! Form::open(['route' => 'action_search_path', 'id' => 'searchForm']) !!}
                    ...
                    @if (!empty($needle))
                        <button class="btn btn-default reset-button equis" type="button" data-location="actions">x</button>
                    @endif
                    {!! Form::close() !!}
                @endif
            </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('name', 'Action') !!}</th>
                        <th class="actions">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                        @foreach ($logActions as $logAction)
                            <tr data-id="{{ $logAction->id }}" class="{{ !empty($logAction->disabled) ? 'disabled' : '' }}">
                                <td>
                                    @if (Auth::user()->isAllowTo('update-action'))
                                        <span data-toggle="tooltip" title="Click to edit">
                                            <a class="x-editable" href="#"
                                               data-pk="{{ $logAction->id }}"
                                               data-name="name"
                                               data-value="{{ $logAction->name }}"
                                               ...
                                               data-type="text"
                                               data-title="Action:"
                                               data-url="{{ route('action_inline_update_path') }}">
                                                {{ $logAction->name }}
                                            </a>
                                        </span>
                                    @else
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('delete-action'))
                                                    <li class="menu-separator"></li>
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="delete" data-id="{{ $logAction->id }}">
        ...
        <div class="row pagination-container clearfix text-center">
            <div class="col-xs-12 col-sm-4">
                <div class="pull-left pagination-info">
                    ...
                    @else
                        @if ($logActions->perPage() > $logActions->total())
                            <span>Showing {!! $logActions->total() !!} {{ $logActions->total() == 1 ? 'item' : 'items' }}</span>
                        @else
                            <span>Showing {{ $logActions->perPage() }} of {!! $logActions->total() !!} {{ $logActions->total() == 1 ? 'item': 'items' }}</span>
                        @endif
                    @endif
                </div>
            </div>
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('action_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('action_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('action_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $logActions->total()])) }}"{{ Request::input('perPage') == $logActions->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $logActions, 'route' => 'action_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'action_delete_path', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('action_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('actions') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
