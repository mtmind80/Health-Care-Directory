    ...
    <header id="topbar">
        <div class="topbar-left">
            <ol class="breadcrumb">
                ...
                <li class="crumb-active">
                    <a href="javascript:void(0)" class="no-link">Internship Types</a>
                </li>
    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-internshiptype'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-internshiptype'))
                    {!! Form::open(['route' => 'internship_type_search_path', 'id' => 'searchForm']) !!}
        ...
        <div class="panel" id="spy7">
            <div class="panel-internshiptype pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable">{!! SortableTrait::link('name', 'Internship Type') !!}</th>
                        <th class="actions">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                        @foreach ($internshipTypes as $internshipType)
                            <tr data-id="{{ $internshipType->id }}" class="{{ !empty($internshipType->disabled) ? 'disabled' : '' }}">
                                <td>
                                    @if (Auth::user()->isAllowTo('update-internshiptype'))
                                        <span data-toggle="tooltip" title="Click to edit">
                                            <a class="x-editable" href="#"
                                               data-pk="{{ $internshipType->id }}"
                                               data-name="name"
                                               data-value="{{ $internshipType->name }}"
                                               ...
                                               data-type="text"
                                               data-title="Internship Type:"
                                               data-url="{{ route('internship_type_inline_update_path') }}">
                                                {{ $internshipType->name }}
                                            </a>
                                        </span>
                                    @else
                                    ...
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update-internshiptype'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="status" data-id="{{ $internshipType->id }}">
                                                            @if (empty($internshipType->disabled))
                                                ...
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-internshiptype'))
                                                    <li class="menu-separator"></li>
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="delete" data-id="{{ $internshipType->id }}">
        ...
        <div class="row pagination-container clearfix text-center">
            <div class="col-xs-12 col-sm-4">
                <div class="pull-left pagination-info">
                    @if (!$internshipTypes->total())
                        <span>There is no item to show.</span>
                    @else
                        @if ($internshipTypes->perPage() > $internshipTypes->total())
                            <span>Showing {!! $internshipTypes->total() !!} {{ $internshipTypes->total() == 1 ? 'item' : 'items' }}</span>
                        @else
                            <span>Showing {{ $internshipTypes->perPage() }} of {!! $internshipTypes->total() !!} {{ $internshipTypes->total() == 1 ? 'item': 'items' }}</span>
                        @endif
                    @endif
                </div>
            </div>
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('internship_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('internship_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('internship_type_list_path', array_merge(Request::query(), ['page' => 0, 'perPage' => $internshipTypes->total()])) }}"{{ Request::input('perPage') == $internshipTypes->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $internshipTypes, 'route' => 'internship_type_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'internship_type_delete_path', 'id' => 'deleteForm']) !!}
...
@section('css-files')
    ...
    <link rel="stylesheet" type="text/css" href="{{ $siteUrl }}/vendor/vendor/editors/xeditable/inputs/internship/internship.css">
...
@section('js-files')
    ...
    <script src="{{ $siteUrl }}/vendor/vendor/editors/xeditable/inputs/internship/internship.js"></script>
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('internship_type_create_path') }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="status"]', function(){
                window.location = "{{ asset('internshiptypes') }}/" + $(this).attr('data-id') + '/togglestatus';
            });
