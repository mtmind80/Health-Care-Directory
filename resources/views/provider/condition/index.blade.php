    ...
    <header id="topbar">
        <div class="topbar-left">
            <ol class="breadcrumb">
                ...
                <li class="crumb-active">
                    <a href="javascript:void(0)" class="no-link">Conditions</a>
                </li>
    ...
    <div id="content" class="animated fadeIn list-items admin-form">
        ...
        <div class="clearfix">
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15">
                <div class="btn-group">
                    @if (Auth::user()->isAllowTo('create-provider-condition'))
            ...
            <div class="col-xs-12 col-sm-6 pl0 ml0 pr0 mr0 mb15 search-container">
                @if (Auth::user()->isAllowTo('search-provider-condition'))
                    {!! Form::open(['url' => route('provider_condition_search_path', ['provider_id' => $provider_id]), 'id' => 'searchForm']) !!}
                    ...
                    @if (!empty($needle))
                        <button class="btn btn-default reset-button equis" type="button" data-location="providerconditions/{{ $provider_id }}">x</button>
                    @endif
                    {!! Form::close() !!}
                @endif
            </div>
        ...
        <div class="panel" id="spy7">
            <div class="panel-body pn">
                <table class="table table-bordered list-table">
                    <thead>
                    <tr>
                        <th class="td-sortable ">{!! SortableTrait::link('conditions.name|provider_conditions.condition_id', 'Condition', ['provider_id' => $provider_id]) !!}</th>
                    ...
                    <tbody>
                        @foreach ($conditions as $condition)
                            <tr data-id="{{ $condition->id }}" class="{{ !empty($condition->disabled) ? 'disabled' : '' }}">
                                <td class="">{!! $condition->condition->name !!}</td>
                                <td class="centered actions">
                                    <ul class="nav navbar-nav">
                                        <li class="dropdown">
                                            <a class="dropdown-toggle" data-toggle="dropdown" href="#"><i class="fa fa-angle-down"></i></a>
                                            <ul class="dropdown-menu animated animated-short flipInX" role="menu">
                                                @if (Auth::user()->isAllowTo('update--provider-condition'))
                                                    <li>
                                                        <a href="javascript:void(0);" class="action" data-action="edit" data-id="{{ $condition->id }}">
                                                            <span class="glyphicons glyphicons-edit"></span>Edit
                                                        </a>
                                                    </li>
                                                @endif
                                                @if (Auth::user()->isAllowTo('delete-provider-condition'))
        ...
        <div class="row pagination-container clearfix text-center">
            ...
            <div class="col-xs-12 col-sm-4 pagination-pages">
                ...
                <select id="pageItems" class="form-control">
                    <option value="{{ route('provider_condition_list_path', array_merge(Request::query(), ['provider_id' => $provider_id, 'page' => 0, 'perPage' => 10])) }}"{{ Request::input('perPage') == 10 ? ' selected' : '' }}>10</option>
                    <option value="{{ route('provider_condition_list_path', array_merge(Request::query(), ['provider_id' => $provider_id, 'page' => 0, 'perPage' => 20])) }}"{{ Request::input('perPage') == 20 ? ' selected' : '' }}>20</option>
                    <option value="{{ route('provider_condition_list_path', array_merge(Request::query(), ['provider_id' => $provider_id, 'page' => 0, 'perPage' => $conditions->total()])) }}"{{ Request::input('perPage') == $conditions->total() ? ' selected' : '' }}>All</option>
                </select>
            </div>
            <div class="col-xs-12 col-sm-4 pull-right pagination-handlers text-right">
                @include('components.pagination.custom', ['paginator' => $conditions, 'route' => 'provider_condition_list_path', 'query' => http_build_query(Input::except(['page', '_token']))])
            </div>
        </div>

        {!! Form::open(['method'=>'DELETE', 'route' => 'provider_condition_delete_path', 'id' => 'deleteForm']) !!}
    ...
    <script>
        $(function(){
            ...
            $('#button-create').click(function(){
                window.location = "{{ route('provider_condition_create_path', ['provider_id' => $provider_id]) }}";
            });
            ...
            $('table.list-table tbody tr td.actions').on('click', 'a.action[data-action="edit"]', function(){
                window.location = "{{ asset('providerconditions') }}/" + $(this).attr('data-id') + '/edit';
            });
