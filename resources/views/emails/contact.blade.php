...
<table width="600" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <td style="color: #484747; background: #FFF; text-align: left; padding: 20px 40px 30px;">
            <p>{{ $name }} {{ ($lang == 'sp') ? 'ha enviado una solicitud de contacto.' : 'has been sent a contact request.' }}</p>
            <p>{!! ($lang == 'sp') ? 'Correo electr&oacute;nico' : 'Email' !!}: {{ $email }}</p>
            <p>{{ ($lang == 'sp') ? 'Asunto' : 'Subject' }}: {{ $subject }}</p>
            <p>{{ ($lang == 'sp') ? 'Mensaje' : 'Message' }}:<br />{{ $text }}</p>
        </td>
    </tr>
    <tr>
